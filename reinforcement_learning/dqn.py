...
for episode in tqdm(range(cfg.n_episode)):
    ...
    for step in range(cfg.n_step):
        ...
        if termination or truncation:
            print(f'episode {episode} ends at {step=}')
            break
        ...
        if step > cfg.train_start_at_n:
            if step % cfg.train_every_n == 0:
                ...
                with torch.no_grad():
                    ...
                    y = data.reward.to(d) + cfg.discount * max_q * (1.0 - data.done.to(d))
                pred = m(data.obs.to(d)).gather(dim=1, index=data.action.view(-1, 1).to(d))
                loss = torch.nn.functional.mse_loss(pred, y)
                print(f'episode {episode} {loss=}')
